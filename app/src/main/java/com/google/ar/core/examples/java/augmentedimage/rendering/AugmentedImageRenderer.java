...
//  private final ObjectRenderer imageFrameLowerRight = new ObjectRenderer();
  ...
  private final ObjectRenderer markerRenderer = new ObjectRenderer();
...
markerRenderer.createOnGlThread(context, "models/andy.obj", "models/andy.png");
//markerRenderer.setMaterialProperties(0.0f, 3.5f, 1.0f, 6.0f);
...
//    };
    ...
    // Render marker, standing on top of the maze
    Pose markerModelLocalOffset = Pose.makeTranslation(
            0.0f,
            0.0f,
            0.0f);
...
//    imageFrameLowerLeft.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, tintColor);
    ...
    anchorPose.compose(markerModelLocalOffset).toMatrix(modelMatrix, 0);
    markerRenderer.updateModelMatrix(modelMatrix, 0.1f); // 0.05f is a Magic number to scale
    markerRenderer.draw(viewMatrix, projectionMatrix, colorCorrectionRgba, tintColor);
  }
